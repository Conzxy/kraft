...
size_t File::Read(void *buf, size_t len) noexcept
{
...
size_t File::Write(const void *buf, size_t len) noexcept
{
  ...
  while (remaining > 0) {
    readn = ::fwrite(p, 1, remaining, fp_);
...
bool File::Fsync() noexcept { return 0 == fsync(fileno(fp_)); }
