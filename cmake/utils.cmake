function(kraft_extract_version)
    file(READ "${CMAKE_CURRENT_LIST_DIR}/kraft/version.h" file_contents)
    string(REGEX MATCH "KRAFT_VER_MAJOR ([0-9]+)" _ "${file_contents}")
    if(NOT CMAKE_MATCH_COUNT EQUAL 1)
        message(FATAL_ERROR "Could not extract major version number from kraft/version.h")
    endif()
    set(ver_major ${CMAKE_MATCH_1})

    string(REGEX MATCH "KRAFT_VER_MINOR ([0-9]+)" _ "${file_contents}")
    if(NOT CMAKE_MATCH_COUNT EQUAL 1)
        message(FATAL_ERROR "Could not extract minor version number from kraft/version.h")
    endif()

    set(ver_minor ${CMAKE_MATCH_1})
    string(REGEX MATCH "KRAFT_VER_PATCH ([0-9]+)" _ "${file_contents}")
    if(NOT CMAKE_MATCH_COUNT EQUAL 1)
        message(FATAL_ERROR "Could not extract patch version number from kraft/version.h")
    endif()
    set(ver_patch ${CMAKE_MATCH_1})

    set(KRAFT_VERSION_MAJOR ${ver_major} PARENT_SCOPE)
    set(KRAFT_VERSION_MINOR ${ver_minor} PARENT_SCOPE)
    set(KRAFT_VERSION_PATCH ${ver_patch} PARENT_SCOPE)
    set(KRAFT_VERSION "${ver_major}.${ver_minor}.${ver_patch}" PARENT_SCOPE)
endfunction()

function (kraft_gen_lib lib)
  #if (NOT ${BUILD_SHARED_LIBS})
  if (KRAFT_BUILD_STATIC_LIBS)
    message(STATUS "Build static library: ${lib}")
    add_library(${lib} STATIC ${ARGN})
    
    if (CMAKE_BUILD_TYPE STREQUAL "Debug")
      set(archive_output_path "${CMAKE_BINARY_DIR}/kraft/lib/debug")
    else ()
      set(archive_output_path "${CMAKE_BINARY_DIR}/kraft/lib/release")
    endif ()
    ...
    if (CMAKE_BUILD_TYPE STREQUAL "Debug")
      set(shared_output_path "${CMAKE_BINARY_DIR}/kraft/${so_output_dir}/debug")
    else ()
      set(shared_output_path "${CMAKE_BINARY_DIR}/kraft/${so_output_dir}")
    endif ()
...
function (kraft_gen_app app_name src_list lib_list)
